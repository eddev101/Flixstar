...
export default function Favorites() {
  ...
  return (
    <div className="page container">
      ...
      {items.length === 0
        ? <div style={{ textAlign: 'center', color: 'var(--text-dim)', marginTop: 60, fontSize: '1.1rem' }}>No favorites yet</div>
          ...
          {items.map(item => {
            ...
            return (
              <div key={item.id} className="result-row">
                <img src={`${IMG}/w185${item.poster_path}`} alt={title} onClick={() => navigate(item.type === 'movie' ? `/movie/${item.id}` : `/tv/${item.id}`)} style={{ cursor: 'pointer' }} />
